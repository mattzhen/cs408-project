...
router.get('/item-details/:item_id', function(req, res, next) {
  ...
  const item = req.db.getItemDetails(req.params.item_id, req.session.user_id);
...
router.post('/item-details/:item_id/toggle', (req, res) => {
  req.db.toggleAcquired(req.params.item_id, req.session.user_id);
  res.redirect('/groceries');
});

router.post('/item-details/:item_id/delete', (req, res) => {
  req.db.deleteItem(req.params.item_id, req.session.user_id);
  res.redirect('/groceries');
});
